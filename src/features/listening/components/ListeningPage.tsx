...
export default function ListeningPage() {
    ...
    const subjects = useRandomSubjects({ filter: sub => sub.object === 'vocabulary' });
    ...
    useEffect(() => {
        ...
        if (isPlaying) {
            for (const subject of subjects) {
                audioQueue.add(() => reverse ? audio.readSubjectTranslation(subject) : audio.playSubjectAudio(subject));
                audioQueue.add(() => new Promise(resolve => setTimeout(resolve, timeToAnswer * 1000)));
                audioQueue.add(() => reverse ? audio.playSubjectAudio(subject) : audio.readSubjectTranslation(subject));
    ...
    }, [audioQueue, subjects, audio, isPlaying, timeBetweenWords, timeToAnswer, reverse]);
