...
export default function VerbConjugator({ subjects }: Props) {
    ...
    const setRandom = useCallback(() => {
        ...
        const isVerb = randomSubject.data.parts_of_speech.some((pos) => pos.includes('verb'));
        const isAdjective = randomSubject.data.parts_of_speech.some((pos) => pos.includes('adjective'));
    ...
    const handleSelectAllVerbs = useCallback(
        (checked: boolean) => {
            const verbKeys = (Object.keys(conjugationContexts) as ConjugationContext[]).filter(
                (key) => !key.startsWith('adj_'),
            );
    ...
    const handleSelectAllAdjectives = useCallback(
        (checked: boolean) => {
            const adjKeys = (Object.keys(conjugationContexts) as ConjugationContext[]).filter((key) =>
                key.startsWith('adj_'),
            );
            const newFilters = { ...conjugationFilters };
