...
export default function QuizPage({ subjects }: Props) {
        ...
        return (
            <div className="container mx-auto p-4">
                ...
                {quizType === 'adjective-type' ? (
                    <AdjectiveQuiz subjects={subjects} wordCount={wordCount} onFinish={handleQuizFinish} />
                ) : null}
                {quizType === 'verb-ending' ? (
                    <VerbEndingQuiz subjects={subjects} wordCount={wordCount} onFinish={handleQuizFinish} />
                ) : null}
                {quizType === 'verb-transitivity' ? (
                    <VerbTransitivityQuiz subjects={subjects} wordCount={wordCount} onFinish={handleQuizFinish} />
                ) : null}
            </div>
        );
    }
        ...
        <div className="container mx-auto p-4 max-w-2xl">
            ...
            <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <Button
                        onClick={() => setQuizType('adjective-type')}
                        primary={quizType === 'adjective-type'}
                        className="min-h-20"
                    >
                        Adjective Type Quiz (な/の/noun)
                    </Button>
                    <Button
                        onClick={() => setQuizType('verb-ending')}
                        primary={quizType === 'verb-ending'}
                        className="min-h-20"
                    >
                        Verb Ending Quiz (Ichidan/Godan)
                    </Button>
                    <Button
                        onClick={() => setQuizType('verb-transitivity')}
                        primary={quizType === 'verb-transitivity'}
                        className="min-h-20"
                    >
